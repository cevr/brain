...
export const allAgentProviderIds = ["claude", "codex"] as const;
...
export class AgentPlatformService extends ServiceMap.Service<
  ...
  {
    readonly getProvider: (id: AgentProviderId) => Effect.Effect<AgentProvider, BrainError>;
    readonly listDetectedSourceProviders: () => Effect.Effect<
      ReadonlyArray<AgentProviderId>,
      BrainError
    >;
    readonly listExecutableProviders: () => Effect.Effect<
      ReadonlyArray<AgentProviderId>,
      BrainError
    >;
    readonly resolveInteractiveProvider: (
      requested?: Option.Option<AgentProviderId>,
    ) => Effect.Effect<AgentProviderId, BrainError>;
    readonly resolveDaemonExecutor: (
      requested?: Option.Option<AgentProviderId>,
    ) => Effect.Effect<AgentProviderId, BrainError>;
  }
>()("@cvr/brain/services/AgentPlatform/AgentPlatformService") {
  static layer: Layer.Layer<AgentPlatformService, never, ConfigService | FileSystem | Path> =
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const getProvider = (id: AgentProviderId) =>
        ...
        const listDetectedSourceProviders = () =>
          Effect.forEach(allAgentProviderIds, (id) =>
            providers[id]
              .detectSource()
        ...
        const resolveRequested = (
          requested: Option.Option<AgentProviderId> | undefined,
        ): Option.Option<AgentProviderId> => requested ?? Option.none();
