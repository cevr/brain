...
- **Daemon**: macOS-only (launchd). Single unified plist (`com.cvr.brain-daemon`) fires at 9, 13, 17, 21. `brain daemon tick` dispatches via `resolveJob()` in `schedule.ts`
- **Daemon schedule**: Fri/Sat skip. Sun 9am: meditate. Mon-Thu 9am: ruminate. Other hours: reflect. Higher-priority jobs suppress lower ones at same slot
